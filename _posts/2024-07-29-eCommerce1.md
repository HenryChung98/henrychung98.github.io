...
For user model, User model inherits AbstractUser and you need to add 'AUTH_USER_MODEL = "[app-name].User"` to settings.py at very bottom.
...
class User(AbstractUser):
    nickname = models.CharField(max_length=15, unique=True) # additional 
...
```python
# settings.py
ACCOUNT_EMAIL_VARIFICATION = "mandatory" # user cannot login until verify email
